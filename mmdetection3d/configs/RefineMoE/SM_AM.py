...
bbox_head_template =dict(
            type='MultiStageMultiBranchBBoxHead',
            stage='center',
            sparsity_branch_num=2,
            if_cls_pred=True,
            in_channels=128,
            grid_size=6,
            num_classes=3,
...
# 将bbox_head_template的stage参数替换为stage_list中的每个元素，得到bbox_head_list
bbox_head_list = [bbox_head_template.copy() for _ in stage_list]
for i, stage in enumerate(stage_list):
    bbox_head_list[i]['stage'] = stage
...
model = dict(
    ...
    # init_cfg=dict(
    #     type='Pretrained',
    #     checkpoint='pretrained/pv_rcnn_kitti-3d-3class_remove_roi.pth'
    # ),
    roi_head=dict(
        type='MMrefineV2RoiHead',
        num_stage=len(stage_list),
        stage_loss_weights=[1.0]*len(stage_list),
        num_classes=3,
        semantic_head=dict(
