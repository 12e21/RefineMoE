...
class TaskDecoupleAllBBoxHead(BaseModule):
    """PVRCNN BBox head.
    ...
    def __init__(
        ...
        super(TaskDecoupleAllBBoxHead, self).__init__(init_cfg=init_cfg)
        self.stage = stage
        assert stage in ['x', 'y', 'z', 'w', 'l', 'h', 'yaw']
    ...
    def forward(self, feats: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        if self.stage=='x':
            bbox_pred = F.pad(bbox_pred, (0, 6), value=0)
        elif self.stage=='y':
            bbox_pred = F.pad(bbox_pred, (1, 5), value=0)
        elif self.stage=='z':
            bbox_pred = F.pad(bbox_pred, (2, 4), value=0)
        elif self.stage=='w':
            bbox_pred = F.pad(bbox_pred, (3, 3), value=0)
        elif self.stage=='l':
            bbox_pred = F.pad(bbox_pred, (4, 2), value=0)
        elif self.stage=='h':
            bbox_pred = F.pad(bbox_pred, (5, 1), value=0)
        elif self.stage=='yaw':
            bbox_pred = F.pad(bbox_pred, (6, 0), value=0)                    
    ...
    def loss(self, with_cls_pred,cls_score: torch.Tensor, bbox_pred: torch.Tensor,
        ...
        if pos_inds.any() == 0:
            ...
            if self.stage == 'x':
                code_weights = [1,0,0,0,0,0,0]
            elif self.stage == 'y':
                code_weights = [0,1,0,0,0,0,0]
            elif self.stage == 'z':
                code_weights = [0,0,1,0,0,0,0]
            elif self.stage == 'w':
                code_weights = [0,0,0,1,0,0,0]
            elif self.stage == 'l':
                code_weights = [0,0,0,0,1,0,0]
            elif self.stage == 'h':
                code_weights = [0,0,0,0,0,1,0]
            elif self.stage == 'yaw':
