...
class  SampleDecoupleSeparateBBoxHead(BaseModule):
    """PVRCNN BBox head.
    ...
    def __init__(
        ...
        super(SampleDecoupleSeparateBBoxHead, self).__init__(init_cfg=init_cfg)
        ...
        self.shared_fc_layer_list = nn.ModuleList()
        ...
        for i in range(self.sparsity_branch_num):
            ...
            shared_fc_layer = self._make_fc_layers(
            ...
            self.shared_fc_layer_list.append(shared_fc_layer)
    ...
    def forward(self, feats_list: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        for idx in range(self.sparsity_branch_num):
            ...
            shared_feats = self.shared_fc_layer_list[idx](feats)
    ...
    def loss(self, cls_score_list: List[torch.Tensor], bbox_pred_list: List[torch.Tensor],
        ...
        Args:
            ...
            stacked_sparsity_scores : Sparsity scores.
        ...
        for branch_idx, (cls_score, bbox_pred) in enumerate(zip(cls_score_list, bbox_pred_list)):
            branch_losses = self._calculate_single_branch_loss(
                ...
                pos_gt_bboxes, reg_mask, label_weights, bbox_weights, stacked_sparsity_scores[branch_idx]
            )
    ...
    def _calculate_single_branch_loss(self, cls_score: torch.Tensor, bbox_pred: torch.Tensor,
        ...
        # multiply sparsity scores to the loss
        label_weights = label_weights * sparsity_scores
        bbox_weights = bbox_weights * sparsity_scores
