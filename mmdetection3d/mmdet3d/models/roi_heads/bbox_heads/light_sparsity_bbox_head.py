...
class  LightSparsityBBoxHead(BaseModule):
    """PVRCNN BBox head.
    ...
    def __init__(
        ...
        super(LightSparsityBBoxHead, self).__init__(init_cfg=init_cfg)
    ...
    def forward(self, feats: torch.Tensor, group_indices: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        # (B * N)
        # pseudo_sparsity_index = torch.randint(low=0, high=1+1, size=(rcnn_batch_size,), dtype=torch.long)
        feats = feats.permute(0, 4, 1, 2, 3).contiguous().view(rcnn_batch_size, -1, 1, 1)
        # (BxN, C*6*6*6)
        shared_feats = self.shared_fc_layer(feats)
        
        # # Initialize output tensors with the correct shape
        # cls_score = torch.zeros((rcnn_batch_size, self.cls_out_channels), device=feats.device)
        # bbox_pred = torch.zeros((rcnn_batch_size, self.reg_out_channels), device=feats.device)
        
        # # Process each sample with the appropriate layer based on its index
        # for idx in range(self.sparsity_branch_num):  # Assuming index ranges from 0 to 2
        #     # Get mask for current index
        #     mask = (group_indices == idx)
        #     # Skip if only one sample with current index for batch norm calculation
        #     if (not mask.any()) or (mask.sum() == 1):
        #         continue
            
        #     # Get features for samples with current index
        #     current_feats = shared_feats[mask]
        #     # Process with corresponding layer
        #     current_cls = self.cls_layer_list[idx](current_feats).transpose(1, 2).contiguous().view(-1, self.cls_out_channels)
        #     current_bbox = self.reg_layer_list[idx](current_feats).transpose(1, 2).contiguous().view(-1, self.reg_out_channels)
            
        #     # Place results back in the correct positions
        #     cls_score[mask] = current_cls
        #     bbox_pred[mask] = current_bbox

        # >>> pseudo soft seb val code >>>
        # Initialize list to store each branch's results
        cls_score_list = []
        bbox_pred_list = []

        # Process all samples with each branch and accumulate results
        for idx in range(self.sparsity_branch_num):
            # Process all samples with the current branch
            current_cls = self.cls_layer_list[idx](shared_feats).transpose(1, 2).contiguous().view(-1, self.cls_out_channels)
            current_bbox = self.reg_layer_list[idx](shared_feats).transpose(1, 2).contiguous().view(-1, self.reg_out_channels)
    ...
    def loss(self, cls_score_list: List[torch.Tensor], bbox_pred_list: List[torch.Tensor],
        ...
        """Computing losses for multiple branches.
        ...
        Returns:
            dict: Computed losses for each branch.
                - loss_cls_branch{i} (torch.Tensor): Loss of classes for branch i.
                - loss_bbox_branch{i} (torch.Tensor): Loss of bboxes for branch i.
                - loss_corner_branch{i} (torch.Tensor): Loss of corners for branch i.
        """
        losses = dict()
        
        # Calculate loss for each branch
        for branch_idx, (cls_score, bbox_pred) in enumerate(zip(cls_score_list, bbox_pred_list)):
        ...
        self._add_averaged_losses(losses, len(cls_score_list))
                
        return losses

    def _add_averaged_losses(self, losses: Dict, num_branches: int) -> None:
        """Add averaged losses across all branches to the losses dict and remove branch-specific losses.
        
        Args:
            losses (Dict): Dictionary of branch-specific losses
    ...
    def _calculate_single_branch_loss(self, cls_score: torch.Tensor, bbox_pred: torch.Tensor,
             rois: torch.Tensor, labels: torch.Tensor,
             bbox_targets: torch.Tensor, pos_gt_bboxes: torch.Tensor,
             reg_mask: torch.Tensor, label_weights: torch.Tensor,
             bbox_weights: torch.Tensor) -> Dict:
