...
class MultiStageMultiBranchBBoxHead(BaseModule):
    """Multi-stage Multi-branch BBox head combining stage-specific refinement with sparsity branches.

    Args:
        stage (str): The refinement stage, one of ['center', 'size', 'direction'].
        if_cls_pred (bool): Whether to predict classification scores.
        ...
        reg_channels (tuple(int)): Out channels of each regression layer.
            Defaults to (256, 256).
        dropout_ratio (float): Ratio of dropout layer. Defaults to 0.3.
        ...
        norm_cfg (dict): Type of normalization method.
            Defaults to dict(type='BN2d', eps=1e-5, momentum=0.1)
        loss_bbox (dict): Config dict of box regression loss.
        loss_cls (dict): Config dict of classifacation loss.
        init_cfg (dict, optional): Initialize config of model.
    ...
    def __init__(
        ...
        super(MultiStageMultiBranchBBoxHead, self).__init__(init_cfg=init_cfg)
        
        # Stage validation
        self.stage = stage
        assert stage in ['center', 'size', 'direction'], f"Invalid stage: {stage}"
        ...
        self.if_cls_pred = if_cls_pred
        self.sparsity_branch_num = sparsity_branch_num
        
        # Loss and coding configurations
        ...
        # Stage-specific regression output size
        if self.stage == 'size':
            bbox_code_size = 3
        elif self.stage == 'center':
            bbox_code_size = 3
        elif self.stage == 'direction':
            bbox_code_size = 1

        self.reg_out_channels = bbox_code_size * cls_out_channels
        ...
        # Adjust input channels for grid-based features
        ...
        # Multi-branch head construction
        self.cls_layer_list = nn.ModuleList()
        self.reg_layer_list = nn.ModuleList()
        
        for i in range(self.sparsity_branch_num):
            # Classification branch (only if needed)
            if self.if_cls_pred:
                cls_layer = self._make_fc_layers(
                    shared_fc_channels[-1],
                    cls_channels,
                    range(1),
                    norm_cfg,
                    out_channels=self.cls_out_channels)
                self.cls_layer_list.append(cls_layer)
            
            # Regression branch
            reg_layer = self._make_fc_layers(
                shared_fc_channels[-1],
                reg_channels,
                range(1),
                norm_cfg,
                out_channels=self.reg_out_channels)
            self.reg_layer_list.append(reg_layer)
    ...
    def forward(self, feats: torch.Tensor) -> Tuple[List[torch.Tensor], List[torch.Tensor]]:
        """Forward multi-stage multi-branch bbox head.

        Args:
            feats (torch.Tensor): Batch point-wise features.

        Returns:
            tuple: (cls_score_list, bbox_pred_list) if if_cls_pred is True,
                   else bbox_pred_list only.
        """
        # Feature preprocessing: (B * N, 6, 6, 6, C) -> (B * N, C*6*6*6, 1, 1)
        ...
        for idx in range(self.sparsity_branch_num):
            # Classification prediction (if enabled)
            if self.if_cls_pred:
                current_cls = self.cls_layer_list[idx](shared_feats).transpose(
                    1, 2).contiguous().view(-1, self.cls_out_channels)
                cls_score_list.append(current_cls)
            
            # Regression prediction
            current_bbox = self.reg_layer_list[idx](shared_feats).transpose(
                1, 2).contiguous().view(-1, self.reg_out_channels)
            
            # Stage-specific padding to maintain 7-dim bbox format
            if self.stage == 'center':
                # Pad [x,y,z] -> [x,y,z,0,0,0,0]
                current_bbox = F.pad(current_bbox, (0, 4), value=0)
            elif self.stage == 'size':
                # Pad [w,l,h] -> [0,0,0,w,l,h,0]
                current_bbox = F.pad(current_bbox, (3, 1), value=0)
            elif self.stage == 'direction':
                # Pad [r] -> [0,0,0,0,0,0,r]
                current_bbox = F.pad(current_bbox, (6, 0), value=0)
            
            bbox_pred_list.append(current_bbox)
    ...
    def loss(self, 
        ...
        """Computing losses for multiple branches with stage-specific weighting.
        
        Args:
            with_cls_pred (bool): Whether classification prediction is available.
            cls_score_list (List[torch.Tensor]): List of scores from each branch.
            bbox_pred_list (List[torch.Tensor]): List of bbox predictions from each branch.
        ...
        Returns:
            dict: Computed losses for each branch and averaged losses.
        """
        losses = dict()
        
        # Calculate loss for each branch
        for branch_idx in range(self.sparsity_branch_num):
            cls_score = cls_score_list[branch_idx] if with_cls_pred else None
            bbox_pred = bbox_pred_list[branch_idx]
            sparsity_scores = stacked_sparsity_scores[branch_idx]
            
            branch_losses = self._calculate_single_branch_loss(
                with_cls_pred, cls_score, bbox_pred, rois, labels, bbox_targets, 
                pos_gt_bboxes, reg_mask, label_weights, bbox_weights, sparsity_scores
            )
            
            # Add branch index to loss keys
            for key, value in branch_losses.items():
                losses[f"{key}_branch{branch_idx}"] = value
        
        # Calculate average loss across all branches
        self._add_averaged_losses(losses, self.sparsity_branch_num)
                
        return losses

    def _calculate_single_branch_loss(self, 
                                    with_cls_pred: bool,
                                    ...
                                    sparsity_scores: torch.Tensor) -> Dict:
        """Calculate losses for a single branch with stage-specific handling."""
        losses = dict()
        rcnn_batch_size = bbox_pred.shape[0]

        # Apply sparsity scores to loss weights
        weighted_label_weights = label_weights * sparsity_scores
        weighted_bbox_weights = bbox_weights * sparsity_scores

        # Classification loss (if enabled)
        if with_cls_pred:
            cls_flat = cls_score.view(-1)
            loss_cls = self.loss_cls(cls_flat, labels, weighted_label_weights)
            losses['loss_cls'] = loss_cls

        # Regression loss with stage-specific weighting
        ...
        if pos_inds.any() == 0:
            ...
            bbox_weights_flat = weighted_bbox_weights[pos_inds].view(-1, 1).repeat(
                1, pos_bbox_pred.shape[-1])
            
            # Stage-specific code weighting
            if self.stage == 'center':
                code_weights = [1, 1, 1, 0, 0, 0, 0]
            elif self.stage == 'size':
                code_weights = [0, 0, 0, 1, 1, 1, 0]
            elif self.stage == 'direction':
                code_weights = [0, 0, 0, 0, 0, 0, 1]
            
            code_weights = torch.tensor(code_weights, dtype=bbox_weights_flat.dtype,
            ...
            # Corner loss
            if self.with_corner_loss:
                pos_sparsity_scores = sparsity_scores[pos_inds]
                ...
                losses['loss_corner'] = (loss_corner * pos_sparsity_scores).mean()

        return losses

    def _add_averaged_losses(self, losses: Dict, num_branches: int) -> None:
        """Add averaged losses across all branches to the losses dict."""
        # Collect all loss types and calculate averages
        loss_types = set()
        for key in losses.keys():
            base_key = key.split('_branch')[0]
            loss_types.add(base_key)
        
        # Calculate average for each loss type
        averaged_losses = {}
        for loss_type in loss_types:
            avg_loss = 0
            for branch_idx in range(num_branches):
                avg_loss += losses[f"{loss_type}_branch{branch_idx}"]
            averaged_losses[loss_type] = avg_loss / num_branches
        
        # Clear the original dictionary and add only the averaged losses
        losses.clear()
        losses.update(averaged_losses)

    def decode_from_rois(self, rois, bbox_pred_list):
        """Decode bbox predictions from ROIs for multiple branches."""
        roi_batch_id = rois[..., 0]
        roi_boxes = rois[..., 1:]  # boxes without batch id
        batch_size = int(roi_batch_id.max().item() + 1)

        # Decode boxes for each branch
        all_branch_results = []
        for bbox_pred in bbox_pred_list:
            ...
            batch_box_preds = self.bbox_coder.decode(local_roi_boxes, bbox_pred)
            ...
            for batch_id in range(batch_size):
                ...
                box_preds_detach = box_preds.detach()
                bbox_list.append(box_preds_detach)
            
            all_branch_results.append(bbox_list)
