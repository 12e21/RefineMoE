...
class DecoupleRefineBBoxHead(BaseModule):
    """PVRCNN BBox head.
    ...
    def __init__(
        ...
        super(DecoupleRefineBBoxHead, self).__init__(init_cfg=init_cfg)
        self.stage = stage
        assert stage in ['center', 'size', 'direction']
    ...
    def forward(self, feats: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        if self.stage=='center':
            # 将bbox_pred的第二个维度扩充到7，原有的3维放在前面，后面的4维补充0
            bbox_pred = F.pad(bbox_pred, (0, 4), value=0)
        elif self.stage=='size':
            # 将bbox_pred的第二个维度扩充到7
            bbox_pred = F.pad(bbox_pred, (3, 1), value=0)
        elif self.stage=='direction':
            # 将bbox_pred的第二个维度扩充到7
