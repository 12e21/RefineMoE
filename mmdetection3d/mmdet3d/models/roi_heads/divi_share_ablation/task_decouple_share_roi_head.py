...
class TaskDecoupleShareRoiHead(Base3DRoIHead):
    ...
    def __init__(self,
        ...
        super(TaskDecoupleShareRoiHead, self).__init__(
            bbox_head=bbox_head,
    ...
    def init_bbox_head(self, bbox_roi_extractor: dict,
        ...
        assert len(bbox_head) == self.num_stage
    ...
    def predict(self, feats_dict: dict, rpn_results_list: InstanceList,
        ...
        point_features, feats_dict['keypoints'][..., 1:], feats_dict['keypoints'][..., 0].int(),
        rois)
        for stage in range(self.num_stage):
            # 新的bbox存放在bbox_results中
            bbox_results = self._bbox_forward(stage,pooled_keypoint_features)
        ...
        if refine_reg_option == "all":
            ...
            merge_bbox_results[:, 3:6] = bbox_results_list[1]['bbox_reg'][:, 3:6]
            merge_bbox_results[:, 6] = bbox_results_list[2]['bbox_reg'][:, 6]
    ...
    def _bbox_forward(self, stage ,pooled_keypoint_features) -> dict:
