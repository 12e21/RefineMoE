...
class LightSparsityBranchRoiHead(Base3DRoIHead):
    ...
    def __init__(self,
        ...
        super(LightSparsityBranchRoiHead, self).__init__(
            bbox_head=bbox_head,
    ...
    def loss(self, feats_dict: dict, rpn_results_list: InstanceList,
        ...
        thresholds = [100]
        thresholds_tensor = torch.tensor(thresholds, device=points_count_list[0].device)
        group_indices_list = []
        for points_tensor in points_count_list:
            group_indices = torch.bucketize(points_tensor, thresholds_tensor)
            group_indices_list.append(group_indices)
        stacked_group_indices = torch.stack(group_indices_list).flatten()
    ...
    def predict(self, feats_dict: dict, rpn_results_list: InstanceList,
        ...
        bbox_results = self._bbox_forward(point_features,
                                          feats_dict['keypoints'], rois, stacked_group_indices)
                                          
        avg_bbox_scores = torch.stack(bbox_results['bbox_scores_list'], dim=0).mean(dim=0)
        avg_reg = torch.stack(bbox_results['bbox_reg_list'], dim=0).mean(dim=0)
    ...
    def _bbox_forward_train(self, seg_preds: torch.Tensor,
                            ...
                            group_indices: torch.Tensor) -> dict:
        ...
        keypoint_features = keypoint_features * seg_preds.sigmoid().max(
            dim=-1, keepdim=True).values
        bbox_results = self._bbox_forward(keypoint_features, keypoints, rois, group_indices)
    ...
    def _bbox_forward(self, keypoint_features: torch.Tensor,
                      keypoints: torch.Tensor, rois: torch.Tensor, group_indices:torch.Tensor) -> dict:
        ...
        pooled_keypoint_features = self.bbox_roi_extractor(
            ...
            rois)
        bbox_score_list, bbox_reg_list = self.bbox_head(pooled_keypoint_features,group_indices)
